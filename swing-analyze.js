...
(function (global) {
  ...
  function analyzeDTL(M, raw, club, k, aspect, shift) {
    ...
    shapes.push({ type: 'band', id: 'planeBand', apex: rBall, a: R1.hands, b: R1.shoulder,
      extend: 2.4, fill: 'rgba(77,212,172,0.13)' });
    shapes.push({ type: 'line', id: 'shaftPlane', from: rBall, to: R1.hands, extend: 2.6,
      color: '#4dd4ac', width: 2.2, label: '샤프트 플레인' });
    shapes.push({ type: 'line', id: 'shoulderPlane', from: rBall, to: R1.shoulder, extend: 2.0,
      color: '#4dd4ac', width: 1.8, dash: [7, 6], label: '어깨 플레인' });
    shapes.push({ type: 'line', id: 'spineP1', from: R1.hip, to: R1.shoulder, extend: 1.35,
      color: '#ffb570', width: 2, label: '어드레스 척추선' });
    shapes.push({ type: 'vline', id: 'buttLine', x: R1.hip.x, color: '#ff8fb3', width: 1.6,
      dash: [5, 5], label: '엉덩이 기준선' });
    shapes.push({ type: 'circle', id: 'headZone', center: R1.head, r: rSpine * 0.22 * k,
      color: '#ffd166', width: 1.6, dash: [4, 4], label: '머리 허용 범위' });
    if (R1.ball) shapes.push({ type: 'dot', id: 'ball', at: R1.ball, r: 5, color: '#ffffff', label: '볼' });
    shapes.push({ type: 'trace', id: 'handPath', points: ['hands'], color: '#ff8fb3', width: 2, label: '손 궤적' });
  ...
  function analyzeFO(M, raw, club, k, aspect) {
    ...
    shapes.push({ type: 'vline', id: 'headLine', x: R1.head.x, color: '#ffd166', width: 1.6, dash: [5, 5], label: '머리 기준선' });
    shapes.push({ type: 'circle', id: 'headZone', center: R1.head, r: rShW * 0.24 * k,
      color: '#ffd166', width: 1.6, dash: [4, 4], label: '머리 허용 범위' });
    shapes.push({ type: 'vline', id: 'leadHipLine', x: R1.leadHip.x, color: '#5aa9e6', width: 1.5, dash: [5, 5], label: '앞쪽 골반선' });
    shapes.push({ type: 'vline', id: 'trailHipLine', x: R1.trailHip.x, color: '#3d7fbf', width: 1.5, dash: [5, 5], label: '뒤쪽 골반선' });
    shapes.push({ type: 'line', id: 'shoulderLine', from: R1.trailShoulder, to: R1.leadShoulder, extend: 1.25,
      color: '#4dd4ac', width: 2, label: '어깨 라인' });
    shapes.push({ type: 'line', id: 'spineFO', from: rHipC, to: rShC, extend: 1.3, color: '#ffb570', width: 2, label: '척추선' });
    if (R1.ball) shapes.push({ type: 'dot', id: 'ball', at: R1.ball, r: 5, color: '#ffffff', label: '볼' });
    shapes.push({ type: 'trace', id: 'handPath', points: ['hands'], color: '#ff8fb3', width: 2, label: '손 궤적' });
    shapes.push({ type: 'trace', id: 'clubPath', points: ['clubhead'], color: '#ff9166', width: 2, label: '헤드 궤적' });
    return { metrics: m, shapes: shapes };
  }
